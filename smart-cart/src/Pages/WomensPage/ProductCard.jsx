...
const ProductCard = ({id,discount,img,price,ratings,'ratings-count':rating_count,size,title,strike_price,brand,setAddedtoCart }) => {
    ...
    const handleCartData = () => {
        ...
        let obj = {id,discount,img,price,ratings,'ratings-count':rating_count,size,title,strike_price,brand,count : 1 };
...
return (
    <Box >
        ...
        <Box _hover = {{cursor : 'pointer', boxShadow : 'rgba(120, 119, 119, 0.2) 0px 8px 24px'}} className={style.productcard} position = {'relative'}>
            ...
            <Box overflow={'hidden'}>
            <Link to = {`/womensPage/${id}`} ><Image _hover ={{transform : 'scale(1.1)'} } transition = 'all 0.4s'  src = {img}></Image></Link>
            </Box>
